...
G1_Subject10 <- "G1_Subject10.csv"
G1_Subject11 <- "G1_Subject11.csv"
G1_Subject12 <- "G1_Subject12.csv"

dataframes <- list(G1_Subject10, G1_Subject11, G1_Subject12)
...
downsampled_190CAN0 <- canbus_data %>%
  ...
  summarize(
    mean.190.Engine.Speed.CAN0 = mean(`X190.Engine.Speed.CAN0`, na.rm = TRUE),
  ...
  mutate(interval_1s = as.numeric(difftime(Time, Time, units = "secs"))) %>%
...
canbus_data_downsampled <- canbus_data_downsampled %>%
  #full_join(downsampled_190CAN0, by = "interval_1s") %>%
  full_join(downsampled_gps, by = "interval_1s") %>%
  full_join(downsampled_location, by = "interval_1s") %>%
  full_join(downsampled_sat, by = "interval_1s")


# Filter combined_data based on the time
cyberattack_data <- canbus_data_downsampled %>%
  filter(interval_1s > 1400 & interval_1s < 1700)
...
speed_comparison <- 
  ...
  geom_line(aes(y = mean.84.Wheel.Based.Vehicle.Speed, color = "Wheel-Based Vehicle Speed"), na.rm = TRUE) +
...
combined_data <- combined_data %>%
  rename(Time = Time.x) %>%
  select(-Time.x.x, -Time.y, -Time.y.y)

...
def driverIDPlot(decodedData, canData, vboxData, EDA, EDATimestamps, HR, HRTimestamps, IBI, IBITimestamps):
    #J1939 CAN Data
    vehicleSpeed = decodedData["65265"]["84"]["data"]

    vehicleSpeedTimestamps = decodedData["65265"]["84"]["time"]

    counter = 0
    vehicleSpeedTimestampsMinutes = []
    for i in range(len(vehicleSpeedTimestamps)):
        if counter == 0:
            firstTimestamp = vehicleSpeedTimestamps[i]
            counter = 1
        vehicleSpeedTimestampsMinutes.append((vehicleSpeedTimestamps[i] - firstTimestamp)/60)

    engineRPM = decodedData["61444"]["190"]["data"]
    engineRPMTimestamps = decodedData["61444"]["190"]["time"]

    counter = 0
    engineRPMTimestampsMinutes = []
    for i in range(len(engineRPMTimestamps)):
        if counter == 0:
            firstTimestamp = engineRPMTimestamps[i]
            counter = 1
        engineRPMTimestampsMinutes.append((engineRPMTimestamps[i] - firstTimestamp)/60)

    #Arduino GPS
    longitude = canData[0]
    latitude = canData[1]
    timestampsLocation = canData[8]

    gpsSpeed = canData[2]
    headingDegree = canData[3]
    timestampHeading = canData[7]

    numSats = canData[4]
    gpsTime = canData[5]
    timestampSats = canData[6]
    ...
    vboxIMUPitch = vboxData[57]
    vboxIMURoll = vboxData[58]
    ...
    fig = make_subplots(
        ...
        subplot_titles=(
            'Wheel Based Vehicle Speed (J1939 SPN 84)', 
            'GPS Speed (Arduino)', 
            'Velocity (VBOX3i)', 
            'Engine RPM (J1939 SPN 190)', 
            'Longitude (Arduino)', 
            'Longitude (VBOX3i)',
            'Latitude (Arduino)', 
            'Latitude (VBOX3i)', 
            'Heading Degree (Arduino)', 
            'Heading (VBOX3i)', 
            'Number of Sats (Arduino)', 
            'Number of Sats (VBOX3i)', 
            'IMU Heading (VBOX3i)',
            'IMU Pitch (VBOX3i)',
            'IMU Roll (VBOX3i)',
            'EDA (Skin Conductance/GSR)',
            'Heart Rate',
            'IBI (Heart Rate Variability)'
        )
    )

    # Add J1939 CAN Data
    fig.add_trace(go.Scatter(x=vehicleSpeedTimestampsMinutes, y=vehicleSpeed), row=1, col=1)
    fig.add_trace(go.Scatter(x=timestampHeading, y=gpsSpeed), row=2, col=1)
    fig.add_trace(go.Scatter(x=vboxTime, y=vboxVelocity), row=3, col=1)
    fig.add_trace(go.Scatter(x=engineRPMTimestampsMinutes, y=engineRPM), row=4, col=1)

    # Add Arduino GPS Data
    fig.add_trace(go.Scatter(x=timestampsLocation, y= longitude), row=5, col=1)
    fig.add_trace(go.Scatter(x=vboxTime, y=vboxLong), row=6, col=1)
    fig.add_trace(go.Scatter(x=timestampsLocation, y=latitude), row=7, col=1)
    fig.add_trace(go.Scatter(x=vboxTime, y=vboxLat), row=8, col=1)
    fig.add_trace(go.Scatter(x=timestampHeading, y=headingDegree), row=9, col=1)
    fig.add_trace(go.Scatter(x=vboxTime, y=vboxHeading), row=10, col=1)
    fig.add_trace(go.Scatter(x=timestampSats, y=numSats), row=11, col=1)

    # Add VBOX3i Data
    fig.add_trace(go.Scatter(x=vboxTime, y=vboxSat), row=12, col=1)
    fig.add_trace(go.Scatter(x=vboxTime, y=vboxIMUHeading), row=13, col=1)
    fig.add_trace(go.Scatter(x=vboxTime, y=vboxIMUPitch), row=14, col=1)
    fig.add_trace(go.Scatter(x=vboxTime, y=vboxIMURoll), row=15, col=1)

    # Add Heart Rate Data
    fig.add_trace(go.Scatter(x=EDATimestamps, y=EDA), row=16, col=1)
    fig.add_trace(go.Scatter(x=HRTimestamps, y=HR), row=17, col=1)
    fig.add_trace(go.Scatter(x=IBITimestamps, y=IBI), row=18, col=1)
    ...
    for i in range(1, 19):
        fig.update_xaxes(title_text='Minutes', row=i, col=1)
    ...
    fig.update_yaxes(title_text='Degrees', row=9, col=1)
    fig.update_yaxes(title_text='Degrees', row=10, col=1)
    fig.update_yaxes(title_text='Degrees', row=13, col=1)
    fig.update_yaxes(title_text='Degrees', row=14, col=1)
    fig.update_yaxes(title_text='Degrees', row=15, col=1)
    fig.update_yaxes(title_text='microsiemens (uS)', row=16, col=1)
    fig.update_yaxes(title_text='Beats per Minute', row=17, col=1)
    fig.update_yaxes(title_text='Seconds Between Heartbeats (s)', row=18, col=1)
...
def main():
    ...
    driverIDPlot(decodedData, canData, vboxData, eda_data_values, eda_timestamps, hr_data_values, hr_timestamps, ibi_data_values, ibi_timestamps)
